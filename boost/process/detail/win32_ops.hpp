...
inline std::string encode_utf8(const std::wstring& wstr)
{
    std::string bytes;
...
{ 
    ...
    BOOST_ASSERT(setup.startupinfo->cb >= sizeof(STARTUPINFOW)); 
    BOOST_ASSERT(!(setup.startupinfo->dwFlags & STARTF_USESTDHANDLES)); 

    boost::scoped_ptr<STARTUPINFOW> si(new STARTUPINFOW); 
