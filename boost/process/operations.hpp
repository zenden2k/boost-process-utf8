...
#elif defined(BOOST_WINDOWS_API) 
    const wchar_t *exts[] = { L"", L".exe", L".com", L".bat", NULL }; 
...
{ 
    ...
    UINT size = ::GetSystemDirectoryW(sysdir, sizeof(sysdir)); 
    if (!size) 
    ...
    exe = detail::encode_utf8(sysdir) + (sysdir[size - 1] != '\\' ? "\\cmd.exe" : "cmd.exe");
    args.push_back("cmd"); 
    args.push_back("/c"); 
    args.push_back(command); 
#endif 
